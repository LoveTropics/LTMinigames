...
public final class SelectPlayerRoleScreen extends Screen {
	private static final Component TITLE = GameLobbyTexts.Ui.SELECT_PLAYER_ROLE.copy()
			.withStyle(ChatFormatting.BOLD, ChatFormatting.UNDERLINE);
	private static final Component TEXT = GameLobbyTexts.Ui.selectRoleMessage(
			GameLobbyTexts.Ui.SELECT_PLAY.copy().withStyle(ChatFormatting.AQUA),
			GameLobbyTexts.Ui.SELECT_SPECTATE.copy().withStyle(ChatFormatting.AQUA)
	);
	...
	protected void init() {
		...
		Button playButton = buttonsHelper.addChild(Button.builder(GameLobbyTexts.Ui.SELECT_PLAY, b -> {
			sendResponse(true);
			onClose();
		}).width(BUTTON_WIDTH).build());
		buttonsHelper.addChild(Button.builder(GameLobbyTexts.Ui.SELECT_SPECTATE, b -> {
			sendResponse(false);
			onClose();
		}).width(BUTTON_WIDTH).build());

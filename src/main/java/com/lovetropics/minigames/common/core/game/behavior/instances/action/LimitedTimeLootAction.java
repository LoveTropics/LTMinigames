	...
	public void register(IGamePhase game, EventRegistrar events) {
		...
		events.listen(GameWorldEvents.BLOCK_DROPS, (player, pos, blockState, blockEntity, tool, drops) -> {
			...
			for (ItemStack item : lootTable.getRandomItems(params)) {
				double x = pos.getX() + 0.5 + Mth.nextDouble(game.random(), -0.25, 0.25);
				double y = pos.getY() + 0.5 + Mth.nextDouble(game.random(), -0.25, 0.25) - EntityTypes.ITEM.getHeight() / 2.0;
				double z = pos.getZ() + 0.5 + Mth.nextDouble(game.random(), -0.25, 0.25);

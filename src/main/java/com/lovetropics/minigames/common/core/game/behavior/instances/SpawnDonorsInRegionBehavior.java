...
public record SpawnDonorsInRegionBehavior(
	...
	public void register(IGamePhase game, EventRegistrar events) throws GameException {
		...
		integrations.get("donations/donations/all/limited", MinecrafterDonor.LIST_CODEC).thenAcceptAsync(result -> {
				if (result.isPresent()) {
					...
					for (MinecrafterDonor donor : donors) {
						SpawnDonorUtils.spawnDonorInRandomRegion(game, donor, regions, scales.orElse(List.of()));
		...
		events.listen(GameLivingEntityEvents.MOB_DROP, (e, d, r) -> {
			if (e.hasData(LoveTropicsAttachments.DONOR)) {
				ItemStack book = new ItemStack(Items.WRITTEN_BOOK, 1);
				final MinecrafterDonor donor = e.getData(LoveTropicsAttachments.DONOR);
				List<String> donorComments = List.of(donor.comments());
				...
				book.set(DataComponents.CUSTOM_NAME, Component.translatable("ltminigames.minigame.escape_race.donorbook.title", donor.getDisplayName(DyeColor.WHITE.getTextColor(), e.getRandom())));
